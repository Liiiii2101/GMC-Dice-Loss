...
from nnunetv2.training.nnUNetTrainer.variants.network_and_loss.nnUNetTrainerSwinUNETR import nnUNetTrainerSwinUNETR
import numpy as np
import torch
...
from nnunetv2.training.loss.compound_losses import DC_SkelREC_and_CE_loss, DC_MatREC_and_CE_loss,DC_MatRECPrecision_and_CE_loss,DC_MatRECPrecisionDSC_and_CE_loss
from nnunetv2.training.loss.deep_supervision import DeepSupervisionWrapper
...
from nnunetv2.training.dataloading.data_loader_3d_mat import nnUNetDataLoader3DMat
...
class nnUNetTrainerSwinUNETRMatRecallPrecisionDSC(nnUNetTrainerSwinUNETR):
    ...
    def _build_loss(self):
        ...
        loss = DC_MatRECPrecisionDSC_and_CE_loss(soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
    ...
    def get_dataloaders(self):
        ...
        if dim == 2:
            ...
            dl_tr = nnUNetDataLoader3DMat(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None, transforms=tr_transforms)
            dl_val = nnUNetDataLoader3DMat(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
