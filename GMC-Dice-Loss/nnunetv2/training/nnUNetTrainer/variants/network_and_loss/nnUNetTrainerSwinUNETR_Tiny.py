...
class nnUNetTrainerSwinUNETR_Tiny(nnUNetTrainerNoDeepSupervision):#(nnUNetTrainer):#(nnUNetTrainerNoDeepSupervision):
    """
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        img_size = (96,
                160,
                160)#configuration_manager.patch_size
        spatial_dims = len(img_size)

        model = SwinUNETR(
            in_channels = num_input_channels,
            out_channels = num_output_channels, #label_manager.num_segmentation_heads,
            #img_size = img_size,
            num_heads = (3, 6, 12, 24),
            norm_name = "instance",
            drop_rate = 0.0,
        ...
        print("Trans Params: {}".format(total_params))
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        #target = torch.concat(target, dim=1)
        output = self.network(data)
        #print(type(output), type(target))
        #print(output.shape, target[0].shape, target[1].shape)
