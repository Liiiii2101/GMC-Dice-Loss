...
class nnUNetTrainernnFormerSkeletonRecall(nnUNetTrainernnFormer):
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        target = batch['target']
        skel = batch['skel']
        #print("target", np.unique(target.cpu().numpy()), "skel", np.unique(skel.cpu().numpy()))
        # import napari
        ...
        #     output = self.network(data)
        #     # del data
        #     print(output.shape, target.shape, skel.shape)
        #     l = self.loss(output, target, skel)
