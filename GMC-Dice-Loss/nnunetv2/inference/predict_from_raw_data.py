    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                out_inter = self.network(torch.flip(x, axes))
                #print(out_inter[0].shape, out_inter[1].shape, out_inter[2].shape)
                if type(out_inter) == list:
                    prediction += torch.flip(out_inter[0], axes)
                else:
                    prediction += torch.flip(out_inter, axes)
